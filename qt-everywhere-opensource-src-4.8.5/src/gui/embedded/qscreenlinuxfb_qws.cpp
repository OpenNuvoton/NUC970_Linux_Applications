...
bool QLinuxFbScreen::connect(const QString &displaySpec)
{
    ...
    do {
        ...
        const char *qwsosd = ::getenv("QWS_OSD");
        if (! qwsosd) {
            break;
        }
        osd_colorkey = strtoul(qwsosd, NULL, 0);
        qDebug("QWS_OSD: 0x%08X\n", osd_colorkey);

        if ( ioctl(d_ptr->fd, IOCTL_GET_OSD_OFFSET, (unsigned long) &osd_offset) < 0 ) {    // ENOIOCTLCMD
            qDebug("No support IOCTL_GET_OSD_OFFSET command\n");
            break;
        }
        
        if (finfo.smem_len <= osd_offset) {
            qDebug("OSD support may not be enabled in kernel configuration.\n");
            break;
        }
        ...
        if (vinfo.bits_per_pixel != 16 ||
            ...
            vinfo.blue.length != 5) {
            qDebug("OSD disabled. No support for non-RGB565 display format.\n");
            break;
        }
        
        /* Open OSD */
        osd_block.cmd = OSD_Open;
        if ( ioctl(d_ptr->fd, OSD_SEND_CMD, (unsigned long) &osd_block) < 0 ) { // ENOIOCTLCMD
            qDebug("No support OSD_SEND_CMD ioctl command\n");
            break;
        }   
        
        /* Set color key */
        osd_block.cmd   = OSD_SetTrans;
        osd_block.color = (osd_colorkey & 0xFFFF);
        if ( ioctl(d_ptr->fd, OSD_SEND_CMD, (unsigned long) &osd_block) < 0 ) { // ENOIOCTLCMD
            qDebug("No support OSD_SEND_CMD ioctl command\n");
            break;
        }
...
void QLinuxFbScreen::disconnect()
{
    ...
    ioctl(d_ptr->fd, OSD_SEND_CMD, (unsigned long) &osd_block);
    osd_block.cmd = OSD_Close;
    ioctl(d_ptr->fd, OSD_SEND_CMD, (unsigned long) &osd_block);
#endif
