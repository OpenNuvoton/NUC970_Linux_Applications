    ...
    if (!setjmp(err->setjmp_buffer)) {
        ...
        if (!quickGray) {
            ...
            while (info->output_scanline < info->output_height) {
                ...
                } else if (info->out_color_space == JCS_RGB && info->output_components == 4) {
                    // FIXME: Output format ARGB32 emitted by Nuvoton-hacked IJG JPEG lib.
                    uchar *in = rows[0] + clip.x() * 4;
                    QRgb *out = (QRgb*)outImage->scanLine(y);
                    for (int i = 0; i < clip.width(); ++i) {
...
bool QJpegHandlerPrivate::readJpegHeader(QIODevice *device)
{
    if(state == Ready)
    {
        ...
        // FIXME: Recommend ARGB32 output format which suits Qt most. This hack doesn't impact original IJG JPEG lib and will get
        //        H/W acceleration with Nuvoton-hacked IJG JPEG lib.
