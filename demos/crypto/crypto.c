...
static void aes_crypt(__u8 *protocol, int encrypt, int use_mtp_key, char *in,
		      int inlen, char *out, const char *key, const char *oiv)
{
	int opfd;
	int tfmfd;
...
static void des_crypt(int is_3des, __u8 *protocol, int encrypt, char *in, int inlen,
		      char *out, const char *key, const char *oiv)
{
	int opfd;
	int tfmfd;
	struct sockaddr_alg sa = {
...
void AES_demo(int use_mtp_key)
{
	...
	char out[BUF_SIZE] = {0};
	char in[BUF_SIZE];
	static const char key[16] = "\x06\xa9\x21\x40\x36\xb8\xa1\x5b\x51\x2e\x03\xd5\x34\x12\x00\x06";
...
void DES_demo(void)
{
	...
	char out[BUF_SIZE] = {0};
	char in[BUF_SIZE];
	static const char key[24] = "\x06\xa9\x21\x40\x36\xb8\xa1\x5b"
...
{
	struct utsname kernel_info;
	const char *check_version = "3.11.0";
	...
	/*
	 *  The following functions are available in NUC970 Linux 3.10 BSP only.
	 *  DES/3DES and crypto_raw supports are obsoleted in NUC970 Linux 5.10 BSP.
	 */
	DES_demo();
	TDES_demo();
	crypto_raw_demo();
