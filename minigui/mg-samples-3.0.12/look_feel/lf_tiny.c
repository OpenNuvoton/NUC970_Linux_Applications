/* 
** $Id: lf_tiny.c 10660 2008-08-14 09:30:39Z weiym $
**
...
static char *iconname[]={"form.ico","failed.ico","mg_help.ico","warning.ico",
                     "excalmatory.ico"};
static BITMAP *checkbutton =NULL, *radiobutton =NULL, *bg_pic =NULL;
#define   CHECK_BUTTON_BMP      "flat_check_button.bmp"
#define   RADIO_BUTTON_BMP      "flat_radio_button.bmp"
#define   BG_BMP                "bg_tiny.bmp"
static char cur_dir[256] ={0};
...
#define  CAPTION_METRIC   30
#define  MENU_METRIC      25
#define  BORDER_METRIC    0
#define  SCROLLBAR_METRIC     6


//window element colors
#define   FGC_ACTIVE_CAPTION              0xFF000000
#define   BGCA_ACTIVE_CAPTION             0xFFE35400
#define   BGCB_ACTIVE_CAPTION             0xFFFF953D
#define   FGC_MENU                        0xFF000000
#define   BGC_MENU                        0xFFFFFFFF
#define   FGC_MSGBOX                      0xFF000000
#define   FGC_TIP                         0xFF000000
#define   BGC_TIP                         0xFFFFFFFF
#define   FGC_ACTIVE_BORDER               0xFFC8D0D4
#define   FGC_INACTIVE_BORDER             0xFFC8D0D4
#define   FGC_INACTIVE_CAPTION            0xFFF8E4D8
#define   BGCA_INACTIVE_CAPTION           0xFFDF967A
#define   BGCB_INACTIVE_CAPTION           0xFFEBB99D
#define   FGC_WINDOW                      0xFF000000
#define   BGC_WINDOW                      0xFFFFFFFF
#define   FGC_3DBOX                       0xFF000000
#define   MAINC_3DBOX                     0xFFD8E9EC
#define   FGC_SELECTED_ITEM               0xFFFFFFFF
#define   BGC_SELECTED_ITEM               0xFFC56A31
#define   BGC_SELECTED_LOSTFOCUS          0xFFD8E9EC
#define   FGC_DISABLED_ITEM               0xFF99A8AC
#define   BGC_DISABLED_ITEM               0xFF000000
#define   FGC_HILIGHT_ITEM                0xFFFFFFFF
#define   BGC_HILIGHT_ITEM                0xFFC56A31
#define   FGC_SIGNIFICANT_ITEM            0xFFFFFFFF
#define   BGC_SIGNIFICANT_ITEM            0xFFC56A31
#define   BGC_DESKTOP                     0xFFC08000

static BOOL InitTinyWindowElementAttrs (PWERENDERER rdr)
{
    rdr->we_metrics[0] = CAPTION_METRIC;
    rdr->we_metrics[1] = MENU_METRIC;
    rdr->we_metrics[4] = BORDER_METRIC;
    rdr->we_metrics[5] = SCROLLBAR_METRIC;

    rdr->we_colors [0][0] = FGC_ACTIVE_CAPTION;
    rdr->we_colors [0][1] = BGCA_ACTIVE_CAPTION;
    rdr->we_colors [0][2] = BGCB_ACTIVE_CAPTION;

    rdr->we_colors [1][0] = FGC_MENU;
    rdr->we_colors [1][1] = BGC_MENU;

    rdr->we_colors [2][0] = FGC_MSGBOX;

    rdr->we_colors [3][0] = FGC_TIP;
    rdr->we_colors [3][1] = BGC_TIP;

    rdr->we_colors [4][0] = FGC_ACTIVE_BORDER;
    rdr->we_colors [4][1] = FGC_INACTIVE_BORDER;

    rdr->we_colors [6][0] = FGC_INACTIVE_CAPTION;
    rdr->we_colors [6][1] = BGCA_INACTIVE_CAPTION;
    rdr->we_colors [6][2] = BGCB_INACTIVE_CAPTION;

    rdr->we_colors [7][0] = FGC_WINDOW;
    rdr->we_colors [7][1] = BGC_WINDOW;

    rdr->we_colors [8][0] = FGC_3DBOX;
    rdr->we_colors [8][1] = MAINC_3DBOX;

    rdr->we_colors [9][0] = FGC_SELECTED_ITEM;
    rdr->we_colors [9][1] = BGC_SELECTED_ITEM;
    rdr->we_colors [9][2] = BGC_SELECTED_LOSTFOCUS;

    rdr->we_colors [10][0] = FGC_DISABLED_ITEM;
    rdr->we_colors [10][1] = BGC_DISABLED_ITEM;

    rdr->we_colors [11][0] = FGC_HILIGHT_ITEM;
    rdr->we_colors [11][1] = BGC_HILIGHT_ITEM;

    rdr->we_colors [12][0] = FGC_SIGNIFICANT_ITEM;
    rdr->we_colors [12][1] = BGC_SIGNIFICANT_ITEM;

    rdr->we_colors [13][1] = BGC_DESKTOP;
...
{
    char   icon_load[256], pic_load[256];
    int    i;

    //InitWindowElementAttrs (renderer);
    InitTinyWindowElementAttrs (renderer);
    ...
    for(i =0;i <5;i++){
        ...
        strcat(icon_load,ICON_PATH);
        strcat(icon_load,iconname[i]);
        renderer->we_icon[0][i] =LoadIconFromFile(HDC_SCREEN, icon_load, 1);
        if(!renderer->we_icon[0][i])
            printf("file %s load error!\n",icon_load);
        renderer->we_icon[1][i] =LoadIconFromFile(HDC_SCREEN, icon_load, 0);
        if(!renderer->we_icon[1][i])
            printf("file %s load error!\n",icon_load);
    }
    ...
    if (!RegisterSystemBitmap (HDC_SCREEN, renderer->name, SYSBMP_RADIOBUTTON))
        return -1;*/
    sprintf(pic_load,"%s",cur_dir);
    strcat(pic_load,ICON_PATH);
    strcat(pic_load,RADIO_BUTTON_BMP);
    //radiobutton =LoadBitmapFromRes (HDC_SCREEN, pic_load);
    radiobutton =calloc(sizeof(BITMAP),1);
    LoadBitmap(HDC_SCREEN,radiobutton,pic_load);
    if(!radiobutton){
        printf("file %s load error!\n",pic_load);
    }
    ...
    sprintf(pic_load,"%s",cur_dir);
    strcat(pic_load,ICON_PATH);
    strcat(pic_load,CHECK_BUTTON_BMP);
    checkbutton =LoadBitmapFromRes (HDC_SCREEN, pic_load);
    if(!checkbutton){
        printf("file %s load error!\n",pic_load);
    }
    ...
    sprintf(pic_load,"%s",cur_dir);
    strcat(pic_load,ICON_PATH);
    strcat(pic_load,BG_BMP);
    bg_pic =LoadBitmapFromRes (HDC_SCREEN, pic_load);
    if(!bg_pic){
        printf("file %s load error!\n",pic_load);
    }
...
static int deinit (PWERENDERER renderer)
{
    ...
    for (i = 0; i < 5; i++) {
        if (renderer->we_icon[0][i])
            DestroyIcon (renderer->we_icon[0][i]);

        if (renderer->we_icon[1][i])
            DestroyIcon (renderer->we_icon[1][i]);
    }

    //UnregisterSystemBitmap (HDC_SCREEN, renderer->name, SYSBMP_RADIOBUTTON);
    sprintf(file,"%s",cur_dir);
    strcat(file,ICON_PATH);
    strcat(file,RADIO_BUTTON_BMP);
    ReleaseRes(Str2Key(file));
    radiobutton =NULL;
    //UnregisterSystemBitmap (HDC_SCREEN, renderer->name, SYSBMP_CHECKBUTTON);
    sprintf(file,"%s",cur_dir);
    strcat(file,ICON_PATH);
    strcat(file,CHECK_BUTTON_BMP);
    ReleaseRes(Str2Key(file));
    checkbutton =NULL;
...
(HWND hWnd, HDC hdc, const RECT* pRect, DWORD color)
{
    ...
    bg_bmp = bg_pic;   //GetSystemBitmapEx (wnd_rdr_tiny.name, BGPIC_CUSTOM);
    ...
    if ((win_info->dwExStyle & WS_EX_LFRDR_CUSTOM1) && 
        ...
        if (0 != calc_we_area (hWnd, HT_TINY_BTNBAR, &rc))
            return;
        FillBoxWithBitmapPart (hdc, pRect->left, pRect->top, RECTWP (pRect), 
...
static void draw_radio_button (HWND hWnd, HDC hdc, const RECT* pRect, int status)
{
	...
	if( w >= 13 && h >= 13){
	    ...
	    FillBoxWithBitmapPart (hdc, box_l, box_t, 13, 13, 0, 0, radio_bmp, 
                    /*status * 12*/
...
{ 
    ...
    else
    {
        if (0 == calc_we_area (hWnd, HT_TINY_BTNBAR, &rc))
        {
            FillBoxWithBitmapPart (hdc, rc.left, rc.top, RECTW (rc),
...
erase_background (HWND hWnd, HDC hdc, const RECT *rect)
{
    ...
    if (bg_bmp){
        ...
        if (win_info->dwExStyle & WS_EX_LFRDR_CUSTOM3){
            /** btn bar is at bottom */
            FillBoxWithBitmapPart (hdc, rect->left, rect->top, RECTWP (rect), 
