 ...
 */
{
	g_jpeg.quality = quality;
	if ( g_jpeg.quality < 7 )
		g_jpeg.quality = 7;
	if ( g_jpeg.quality > 99 )
		g_jpeg.quality = 105;

	g_jpeg.quality /= 7;
	g_jpeg.quality = 16 - g_jpeg.quality;
	g_jpeg.eparam.qscaling = (unsigned char)g_jpeg.quality;
...
jpeg_set_defaults (j_compress_ptr cinfo)
{
	...
	{
		_demo_uninit_hwjpeg();
		WARNMS( cinfo, JWRN_11 );
		ERREXIT( cinfo, JERR_BAD_STATE );
	}

	if ( ((cinfo->image_width * cinfo->image_height * 2) > g_jpeg.eparam.src_bufsize)
		|| (cinfo->in_color_space != JCS_YCbCr) )
	{
///		printf( "### encode - Error: raw image size must less equal than 640 * 480!\n" );
///		ERREXIT( cinfo, JERR_BAD_STATE );
		TRACEMS1( cinfo, 1, JTRC_10, g_jpeg.eparam.src_bufsize );
...
///	g_jpeg.eparam.decInWait_buffer_size	= 100*1024;
	...
	g_jpeg.eparam.encode_width			= cinfo->image_width;
	g_jpeg.eparam.encode_height			= cinfo->image_height;
	...
	TRACEMS2( cinfo, 1, JTRC_11, g_jpeg.eparam.encode_width, g_jpeg.eparam.encode_height );
